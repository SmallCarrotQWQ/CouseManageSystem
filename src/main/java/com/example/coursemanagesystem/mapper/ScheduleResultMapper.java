...
public interface ScheduleResultMapper {
    @Insert("INSERT INTO schedule_result (schedule_id, course_id, course_name, class_name, course_time, total_hours, specific_time, location) " +
            "VALUES (#{scheduleId}, #{courseId}, #{courseName}, #{className}, #{courseTime}, #{totalHours}, #{specificTime}, #{location})")
    int insert(ScheduleResult scheduleResult);

    @Update("UPDATE schedule_result SET course_name = #{courseName}, class_name = #{className}, " +
            "course_time = #{courseTime}, total_hours = #{totalHours}, specific_time = #{specificTime}, location = #{location} " +
            "WHERE schedule_id = #{scheduleId} AND course_id = #{courseId}")
    int updateScheduleResult(ScheduleResult scheduleResult);

    @Update("UPDATE schedule_result SET specific_time = #{specificTime}, location = #{location} " +
            "WHERE schedule_id = #{scheduleId} AND course_id = #{courseId}")
